    ...
    if (action === 'generatePlan') {
        ...
        const prompt = `
            ...
            **주요 지침:**
            ...
            4.  **일정 내용:** 각 활동 및 방문 장소에 대한 구체적인 설명, 예상 소요 시간, 팁 등을 **다섯 문장 이상**으로 아주 상세하게 작성해주세요.
            5.  **동선:** 논리적이고 효율적인 동선으로 일정을 계획하고, 모든 필수 방문 장소를 포함해야 합니다.
            6.  **출력 형식:** 결과는 다른 설명이나 markdown 포맷 없이, 아래와 같은 순수한 JSON 형식으로만 반환해주세요.
            ...
            **JSON 출력 형식 예시:**
            {
              ...
              "transportationGuide": "교통편 가이드 상세 지침: 일정에 맞춰 각 구간별 최적의 교통편(예: 공항 리무진, JR 하루카, 난카이 라피트, 신칸센, 사철 등)을 구체적으로 추천해야 합니다. 예상 요금, 소요 시간, 노선, 그리고 여행에 유용한 교통 패스(예: 주유패스, 간사이 쓰루패스, JR 간사이 미니패스 등)의 종류, 가격, 사용법, 구매처를 상세히 포함하여 실질적인 도움이 되도록 작성해야 합니다.",
